...
export class HttpTasksService {
  private readonly logger = new Logger(HttpTasksService.name);
  ...
  async failedJobHandle() {
    ...
    this.logger.log(
      `[${new Date(Date.now()).toUTCString()}] http Failed job total size: ${jobs.length}`,
    );
    ...
    this.logger.log(
      `[${new Date(Date.now()).toUTCString()}] http job total retry size: ${jobs.length}`,
    );
  }
}
